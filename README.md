...
1. **[Download the latest release](https://github.com/abu/kivio/releases/latest)** (DMG for macOS, MSI / NSIS for Windows).
...
- **v2.6.6** — Release packaging update for document-heavy Chat workflows. `run_python` can now mount readable local files directly into the Pyodide sandbox, with a larger input limit for PDF/Word/Excel analysis while keeping write/edit protections unchanged. The installer packaging also carries the local Pyodide runtime and common data/document packages (`numpy`, `pandas`, `matplotlib`, `scipy`, `sympy`, `scikit-learn`, `statsmodels`, `pillow`, `seaborn`, `openpyxl`, `xlrd`, and `micropip`) so normal document analysis does not depend on first-run CDN downloads. Model providers now default to Chat tools support and the old provider toggle is no longer shown.
- **v2.6.5** — Refined the AI client window for Windows with a frameless layout, custom capsule-style minimize/maximize/close controls, and safer titlebar spacing across chat, settings, and assistant-center views. The packaged app now also includes built-in `pdf`, `docx`, and `xlsx` skills so document attachments can activate the expected local workflows without requiring users to install those skills separately.
...
See [GitHub Releases](https://github.com/abu/kivio/releases) for the full history. Auto-update checks for new versions on launch and points you here.
...
1. **[下载最新版](https://github.com/abu/kivio/releases/latest)**（macOS 选 DMG，Windows 选 MSI 或 NSIS）
...
- **v2.6.6** —— 面向文档分析工作流的打包更新。`run_python` 现在可以把可读取的本地文件直接挂载到 Pyodide 沙箱，并提高 PDF / Word / Excel 等文档输入上限，同时不放宽写入/编辑保护。安装包也会带上本地 Pyodide 运行时和常用数据/文档包（`numpy`、`pandas`、`matplotlib`、`scipy`、`sympy`、`scikit-learn`、`statsmodels`、`pillow`、`seaborn`、`openpyxl`、`xlrd`、`micropip`），普通文档分析不再依赖首次运行时从 CDN 下载。模型供应商默认支持 Chat tools，旧的供应商开关已移除。
- **v2.6.5** —— 优化 Windows AI 客户端窗口：使用无边框布局，加入自绘胶囊式最小化 / 最大化 / 关闭按钮，并让聊天、设置、助手中心等界面都避开右上角标题栏控制区。安装包现在也内置 `pdf`、`docx`、`xlsx` 三个文档 Skill，用户分析文档附件时不需要再单独安装这些 Skill。
...
完整历史见 [GitHub Releases](https://github.com/abu/kivio/releases)。应用启动时会自动检查更新，发现新版会指向这里。
