...
///   cache_creation`（四者不相交）。**不**用 ABU Agent 的 `total_tokens`——它对 Anthropic 只存
...
mod tests {
    ...
    #[test]
    fn anthropic_ignores_abu_agent_total_tokens_missing_cache() {
        // 即便 total_tokens 存在（ABU Agent 对 Anthropic 只填 input+output、漏 cache），也不能用它，
        // 必须显式加 cache。
