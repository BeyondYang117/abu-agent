# ABU Agent Desktop v2.9.0

桌面 AI 助手 · macOS (Apple Silicon) + Windows。Desktop AI assistant for macOS (Apple Silicon) and Windows.

## 下载 / Downloads

- **Windows** — `ABU Agent.Desktop_2.9.0_x64-setup.exe`（NSIS 安装包 / installer）
- **macOS** — `ABU Agent.Desktop_2.9.0_aarch64.dmg`（Apple Silicon，未签名 / unsigned）
...
- **ABU Agent Chat** —— 不再继承 Agent 人设和 shell 长文；发送路径挂上知识库；自定义 / 助手文本不能改写 Chat 契约；dock、技能和 `/plan` 在 Chat 里保持隐藏。
...
- **ABU Agent Chat** — no longer inherits Agent identity or the shell essay; the send path mounts the knowledge base; custom / assistant text cannot override the Chat contract; the dock, skills, and `/plan` stay hidden in Chat.
...
完整变更 / Full changelog: https://github.com/abu/abu-agent-desktop/compare/v2.8.9...v2.9.0
