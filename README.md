...
- **v2.7.1** — Kivio Code and platform release: the headline is **Kivio Code**, a brand-new terminal coding agent — a Rust CLI/TUI with a headless `-p` print mode and an interactive shell (differential line renderer, tool cards, model/skill selectors, session resume, JSONL-per-session store), build/plan modes with automatic switching (complex tasks move to plan) and layered context governance, context compaction (manual `/compact` plus overflow auto-retry) with a usage gauge, a Pi-parity tool set (slim 7-tool file/exec suite, fuzzy multi-edit, gitignore-aware grep/find, bash full-output offload, web search, image input + vision mixer, one-time full-disk session consent), and MCP/Skills integration with a project `.kivio/config.json` merged over global config, a settings tab, and `/init` `/mcp` `/skill` `/settings` `/new` `/copy` commands. Chat gains external CLI agent integration (Claude Code, codex, pi, hermes, …) over RPC with persistent ACP/codex sessions, cross-restart resume, a thinking-level selector, and slash-command discovery; a rebuilt assistants system with four built-in assistants (writer/coder/researcher/data), a chat-builder, and a dedicated Skill page in a unified Skill/Assistant frame; an OpenAI Responses API provider; a runtime-aware permission capsule; and patient 429 retry with honest degrade messaging. The chat title bar is redesigned into flat ghost controls of a unified size, split between a left control group (runtime / model / permission) and a right status group (context / todo) with stronger sidebar separation, the Assistants and Skills pages now share the chat's flat white surface, and chat UX is polished across three motion phases (jump-to-bottom button, unified press feedback, springier send, smoother streaming/sidebar/theme transitions). Lens adds a "Continue in AI client" button — shown when *send to AI client* is off — that syncs the full multi-turn Lens conversation plus the screenshot to the chat client as a pre-seeded conversation, and preserves Markdown structure in direct-vision translation. Windows gets a native DWM border with an opaque chat window. Apple Silicon DMG was built locally; GitHub Actions built the Windows NSIS (.exe) installer.
