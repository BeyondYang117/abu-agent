...
本目录（`website/`）是 ABU Agent 官网静态站，已部署在下面这台服务器上。
...
- 网站文件目录（服务器）：`/opt/domain-gateway/sites/abu-agent-desktop/`
- Caddy 配置：`/opt/domain-gateway/Caddyfile`（本站的块见 `abu-agent-desktop.xyz` / `www.abu-agent-desktop.xyz`）
- compose：`/opt/domain-gateway/docker-compose.yml`（挂载 `./sites:/srv:ro`）
- 同机还跑着 `sub2api` + `api.zmfooogreencloud.xyz`，**别动**。

## 域名

- `abu-agent-desktop.xyz`（阿里云注册 + 解析）
...
```bash
ABU_AGENT_DEPLOY_HOST=user@example.com \
ABU_AGENT_DEPLOY_DEST=/path/to/site \
./website/deploy.sh
```
