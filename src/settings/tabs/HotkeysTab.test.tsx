 ...
 *   2. 清除按钮走对应的 updater（updateSettings / ScreenshotAnnotate / Lens 三个不同函数）
 */
...
describe('HotkeysTab', () => {
  it('每个快捷键行显示各自 settings 字段的值（不串行）', () => {
    ...
    // 四个值的数字互不相同，任一 props 接错都会让某个数字缺失或重复。
    for (const digit of ['2', '3', '7', '8']) {
  ...
  it('渲染全部快捷键行', () => {
    renderTab()
    // 每行一个「录制」按钮
    expect(screen.getAllByRole('button', { name: t.hotkeyRecord })).toHaveLength(4)
  })
  ...
  it('检查冲突：有冲突显示配对文案', async () => {
    ...
    expect(
      screen.getByText(
        t.hotkeyCheckPair
          .replace('{a}', t.chatHotkeyLabel)
          .replace('{b}', t.closeChatHotkeyLabel)
