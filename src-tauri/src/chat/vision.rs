...
/// 不落盘）；② 纯文本主模型 → 落临时文件 `abu-agent-mcpimg-<uuid>.<ext>` 走辅助视觉
...
pub(super) async fn attach_image_artifacts_for_model(
    ...
    for (artifact, bytes) in &accepted {
        ...
        let path = std::env::temp_dir().join(format!("abu-agent-mcpimg-{}.{ext}", Uuid::new_v4()));
