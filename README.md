...
1. **[下载最新版](https://github.com/abu/kivio/releases/latest)** —— macOS：Apple Silicon `.dmg` · Windows：NSIS `-setup.exe`，或解压即用的 `-portable.zip`。
...
完整历史见 [GitHub Releases](https://github.com/abu/kivio/releases)。
...
1. **[Download the latest release](https://github.com/abu/kivio/releases/latest)** — macOS: Apple Silicon `.dmg` · Windows: NSIS `-setup.exe` or unzip-and-run `-portable.zip`.
