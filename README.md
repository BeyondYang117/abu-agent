...
- **v2.7.2** — Settings backup: export/import all settings to a JSON file, with an API-key show/hide toggle. Translation polish: input-translation prompts gain a target-language lock; screenshot and selection translation prompts are split into independently editable pieces with improved rendering; and the prompt editor is unified to a prefilled + restore-default pattern. Fixes floating-card and main-translator window shadows being clipped at the window edge, plus internal over-engineering cleanup. The Apple Silicon DMG is built via GitHub Actions; the Windows NSIS `.exe` is built locally and uploaded.
...
- **v2.7.2** —— 设置备份：一键导出/导入全部设置为 JSON 文件，并为 API Key 增加显示/隐藏切换。翻译打磨：输入翻译提示词新增目标语种锁定；截图与选中翻译提示词拆分为可独立编辑的片段并优化渲染；提示词编辑统一为「预填 + 恢复默认」。修复浮动卡/主翻译窗投影被窗口边缘裁切；清理内部过度设计（删死脚本与依赖、合并重复 helper、收敛缓存与用量记录）。macOS DMG 由 GitHub Actions 构建，Windows NSIS 本地构建并上传。
