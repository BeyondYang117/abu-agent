...
屏幕上的字、框选的画面、本机已装的编码 CLI，不必拆成三套软件。ABU Agent 把它们收进同一个托盘应用：热键随时呼出，对话走你自己的供应商。

- **一个循环，多种宿主** — 内置 Agent、只读研究用的 ABU Agent Chat、以及本机外部 CLI（Claude Code、Codex、Cursor、OpenCode、Gemini、Kimi、Pi、Hermes、Grok、DeepSeek Harness），共用同一套聊天界面
...
完整记录见 [Releases](https://github.com/BeyondYang117/abu-agent/releases) · 当前版本说明：[v0.1.6](docs/releases/v0.1.6.md)

### 聊天与 Agent

- 工具循环、子代理、Skills、MCP、知识库、附件；一问多模型对比
- 三种运行时：ABU Agent Agent（完整工具）、ABU Agent Chat（检索 / 抓取 / 知识库等只读能力）、外部 CLI
...
1. 安装并打开 ABU Agent，完成首次引导（供应商 + 热键）。
...
安装包未签名。请右键 → 打开，或执行 `xattr -cr "/Applications/ABU Agent Desktop.app"`。截图与替换翻译还需要屏幕录制权限。
