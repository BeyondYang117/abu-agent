...
6. Build the macOS DMG (`.dmg`) locally on Apple Silicon, then upload it to the tag's release:
   ...
   gh release upload vX.Y.Z "src-tauri/target/release/bundle/dmg/Kivio Desktop_X.Y.Z_aarch64.dmg" --repo abu/kivio
   ```
...
9. **Replace the CI-generated release body with hand-written bilingual notes.** The
   ...
   link:
   ```bash
   gh release edit vX.Y.Z --repo abu/kivio --notes-file notes.md
   ```
