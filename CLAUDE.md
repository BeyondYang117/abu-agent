...
**Binding contracts live in `.trellis/spec/`, not here.** This file is the map; the spec dir holds the enforceable details. Start from `.trellis/spec/guides/index.md` and `.trellis/spec/frontend/index.md`, and read the relevant doc before editing that area — notably `guides/external-cli-agents.md` (required before touching `external_agents/`), `guides/subagent-role-definitions.md` (required before touching `agents/` or sub-agent tool narrowing), `guides/mcp-client.md` (required before touching `mcp/conn.rs` / `mcp/manager.rs` / `mcp/result.rs` or bumping `rmcp`), `frontend/builtin-web-search.md`, `frontend/state-management.md`, `frontend/resource-lifecycle.md`, and `frontend/component-guidelines.md`.
...
- **`mcp/`** — MCP client (`conn.rs`, official `rmcp` SDK — the only wire implementation), persistent connection manager (`manager.rs`), result mapping (`result.rs`), and the unified `ChatToolDefinition` tool registry (`native_registry` + external servers). Contract: `.trellis/spec/guides/mcp-client.md`.
...
- `rmcp` (pinned `3.0.0`, Apache-2.0) — official MCP Rust SDK; the only MCP wire implementation. **Version is pinned without a caret on purpose** — 3.x is still evolving breakingly; bumping it is its own task (see the rmcp-defaults checklist in `.trellis/spec/guides/mcp-client.md`).
...
议题走 GitHub Issues（`abu/kivio`），用 `gh` CLI 读写。见 `docs/agents/issue-tracker.md`。
