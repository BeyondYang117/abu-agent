...
<p align="center">
  <a href="https://github.com/abu/kivio/releases/latest"><img src="https://img.shields.io/github/v/release/abu/kivio?style=flat-square&color=4f46e5&label=release" alt="Latest Release"></a>
...
<p align="center">
  <a href="https://github.com/abu/kivio/releases/latest"><strong>Download</strong></a>
  &nbsp;·&nbsp;
...
- **v2.7.2** — Memory & performance: switched to a system font stack instead of the embedded Noto Serif SC (~40 MB less renderer memory per window), destroy Lens/quick-translate webviews on close to reclaim renderer memory on Windows, delayed overlay webview creation, released overlay memory before hiding, capped/evicted the sub-agent table, and stopped double-storing `api_messages` on final messages or sending them redundantly to the frontend. New **Connectors**: an OAuth connector framework with a settings page and per-tool toggles — GitHub/Composio/custom (token), Notion (PKCE+DCR+loopback with token refresh), and Linear/Sentry/Atlassian. Settings: export/import all settings as JSON with an API-key show/hide toggle, model-library brand icons, custom linear SVG nav icons, and option dedup/regrouping. Chat: per-type icons on tool calls, reworked web-search icon and larger tool-call blocks, native tools enabled by default with auto-approval. Translation: input-translation target-language lock, screenshot/selection prompts split into independently editable pieces that preserve Markdown structure, prompt editor unified to prefilled + restore-default. Plus Lens robustness fixes (cold-rebuild selection/text loss, no spurious translate after a screenshot), Windows fixes (exit EcoQoS throttling so global hotkeys survive idle, read latest PATH from the registry, no subprocess terminal popup, Kivio Code terminal VT input), macOS PATH completion for local-CLI scanning, the GPL-3.0 license, a window-shadow clipping fix, and dead-code/over-engineering cleanup. The Apple Silicon DMG is built via GitHub Actions; the Windows NSIS `.exe` is built locally and uploaded.
...
<p align="center">
  <a href="https://github.com/abu/kivio/releases/latest"><strong>下载</strong></a>
  &nbsp;·&nbsp;
  <a href="#截图">截图</a>
  &nbsp;·&nbsp;
  <a href="#更新日志">更新日志</a>
...
- **v2.7.2** —— 内存与性能：改用系统字体栈替代内嵌 Noto Serif SC（每窗口省 ~40MB 渲染器内存）；Windows 上 Lens/快速翻译关闭即销毁 webview 以回收渲染器内存、延迟创建浮层 webview、隐藏前先释放浮层内存；子 agent 表加上限驱逐，final 消息不再双存 `api_messages`、也不再冗余下发前端。新增**连接器（Connectors）**：OAuth 连接器框架 + 设置页 + 逐工具开关——GitHub/Composio/自定义（token）、Notion（PKCE+DCR+loopback，token 刷新）、Linear/Sentry/Atlassian。设置：一键导出/导入全部设置为 JSON + API Key 显隐切换、模型库品牌图标、自定义线性 SVG 导航图标、选项去重归位。Chat：工具调用按类型显示图标、联网搜索图标重做与工具块放大、原生工具默认启用 + approval 默认 auto。翻译：输入翻译语种锁定、截图/选中翻译提示词拆分为可独立编辑并保留 Markdown 结构、提示词统一为「预填 + 恢复默认」。另含 Lens 健壮性修复（冷重建选区/文本丢失、截图后误触发翻译）、Windows 修复（退出 EcoQoS 节流修空闲后热键失灵、从注册表读最新 PATH、消除子进程终端弹窗、Kivio Code 终端 VT 输入）、macOS 打包版补全 PATH、采用 GPL-3.0、窗口投影裁切修复、死代码/过度设计清理。macOS DMG 由 GitHub Actions 构建，Windows NSIS 本地构建并上传。
